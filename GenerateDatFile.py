...
def GenerateDatFile(screen, locFile=None, outPath=None, fixScreen=True):
	...
	if outPath:
		datFile = os.path.join(outPath, datFile)


	# Check the dat file to see if we have added custom strings already
	if datFileExists := os.path.isfile(datFile):
	...
	with open(datFile, 'w') as file:
		if datFileExists:	print(f'[INFO] Overwriting: "{datFile}"')
		else:				print(f'[INFO] Generating: "{datFile}"')
